<x-organizer.organizer-layout title="Event Tickets">
        ...
        <div class="bg-[#063970] text-white rounded-xl shadow-md p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-blue-100 text-sm mt-1 opacity-90">Manage all ticket types for your events</p>
                </div>

                <div class="flex flex-wrap items-center gap-3">
                    <form method="GET" action="{{ route('org.event-tickets.index') }}" class="flex gap-2 min-w-[240px]">
                        <input type="text" name="search" value="{{ request('search') }}"
                               placeholder="Search by ticket name, event..."
                    ...
                    <a href="{{ route('org.event-tickets.create') }}"
                ...
                <table class="w-full min-w-[900px] hidden lg:table">
                    <thead class="bg-gray-50">
                        <tr class="text-xs font-semibold text-gray-600 uppercase tracking-wide">
                            <th class="px-5 py-3.5 text-left w-24">Ticket ID</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                            ...
                            <tr class="hover:bg-orange-50/30 transition-colors">
                                ...
                                <td class="px-5 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('org.event-tickets.edit', $ticket) }}"
                                           class="px-3.5 py-1.5 bg-[#063970] hover:bg-[#063970]/90 text-white text-xs font-medium rounded-md transition shadow-sm">
                                            Edit
                                        </a>

                                        <form action="{{ route('org.event-tickets.destroy', $ticket) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-14 text-gray-500">
                    ...
                    @forelse($tickets as $ticket)
                        <div class="p-4 hover:bg-orange-50/30 transition">
                            ...
                            <div class="flex gap-2">
                                ...
                                <form action="{{ route('org.event-tickets.destroy', $ticket) }}" method="POST" class="flex-1">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            onclick="return confirm('Delete ticket #{{ $ticket->id }} ?')"
