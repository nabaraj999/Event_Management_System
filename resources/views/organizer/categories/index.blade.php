<x-organizer.organizer-layout title="Event Categories">
            ...
            <div class="bg-[#063970] text-white px-6 py-5 lg:px-8 lg:py-6">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <p class="opacity-90 text-sm lg:text-base mt-1">Manage your event categories</p>
                    </div>

                    <!-- Search + Add Button -->
                    <form method="GET" action="{{ route('org.categories.index') }}" class="flex flex-col sm:flex-row gap-3">
                        ...
                        <div class="flex gap-3">
                            ...
                            <a href="{{ route('org.categories.create') }}"
                            ...
                            <tr class="bg-white border-b hover:bg-orange-50 transition">
                                ...
                                <td class="px-6 py-5 text-center">
                                    <div class="flex items-center justify-center space-x-2">
                                        <a href="{{ route('org.categories.edit', $category) }}"
                                           class="px-4 py-2 bg-[#063970] text-white rounded-lg hover:bg-[#052e5c] transition text-xs font-medium">
                                            Edit
                                        </a>
                                        <form action="{{ route('org.categories.destroy', $category) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                ...
                <div class="lg:hidden">
                    @forelse($categories as $category)
                        <div class="m-4 p-5 bg-white border border-gray-200 rounded-xl shadow hover:shadow-lg transition">
                            ...
                            <div class="flex gap-3">
                                ...
                                <form action="{{ route('org.categories.destroy', $category) }}" method="POST" class="flex-1">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                                            onclick="return confirm('Delete {{ addslashes($category->name) }}?')"
                    ...
                    @empty
                        <div class="m-6 p-10 text-center text-gray-500 bg-gray-50 rounded-xl">
                            ...
                            <!-- FIXED: org route -->
                            <a href="{{ route('org.categories.create') }}" class="text-[#FF7A28] font-bold underline">
