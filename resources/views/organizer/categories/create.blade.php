<x-organizer.organizer-layout title="Create New Category">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white shadow-2xl rounded-xl overflow-hidden">
            ...
            <form action="{{ route('org.categories.store') }}" method="POST" class="p-8 space-y-8">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                              class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#FF7A28]">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <select name="icon_type" id="icon_type" onchange="toggleIconInput()"
                                class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#FF7A28]">
                            <option value="fontawesome" selected>Font Awesome</option>
                            <option value="heroicon">Heroicons</option>
                    ...
                    <div id="icon_name_wrapper">
                        ...
                        <input type="text" name="icon_name" value="{{ old('icon_name') }}"
                               ...
                               class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#FF7A28]">
                        <p class="text-xs text-gray-500 mt-1">Font Awesome: <code>fa-solid fa-heart</code> | Heroicon: <code>heart</code></p>
                    </div>
                </div>

                <div id="custom_svg_wrapper" class="hidden">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Custom SVG Code</label>
                    <textarea name="custom_svg" rows="6" class="font-mono text-xs w-full px-4 py-3 border rounded-lg">{{ old('custom_svg') }}</textarea>
                </div>
                ...
                <div class="flex justify-end space-x-4 pt-6 border-t">
                    <a href="{{ route('org.categories.index') }}" class="px-8 py-3 border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</a>
