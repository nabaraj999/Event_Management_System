        ...
        <div class="bg-[#063970] text-white rounded-xl shadow-md p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-blue-100 text-sm mt-1 opacity-90">All ticket bookings for your events</p>
                </div>

                <div class="flex flex-wrap items-center gap-3">
                    <form method="GET" action="{{ route('org.bookings.index') }}" class="flex gap-2 min-w-[300px]">
                        <input type="text" name="query" value="{{ request('query') }}"
                               placeholder="Search by ID, name, email, phone, token..."
                ...
                <table class="w-full min-w-[900px] hidden lg:table">
                    <thead class="bg-gray-50">
                        <tr class="text-xs font-semibold text-gray-600 uppercase tracking-wide">
                            <th class="px-5 py-3.5 text-left w-20">ID</th>
                            <th class="px-5 py-3.5 text-left">Attendee</th>
                        ...
                        @forelse($bookings as $booking)
                            <tr class="hover:bg-orange-50/30 transition-colors">
                                ...
                                <td class="px-5 py-4">
                                    ...
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $booking->email }}</div>
                                </td>

                                <td class="px-5 py-4 font-medium text-gray-900">
                                    {{ Str::limit($booking->event?->title ?? '—', 45) }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-14 text-gray-500">
                                    ...
                                    <p class="text-sm mt-1.5">Bookings will appear here once tickets are sold</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
